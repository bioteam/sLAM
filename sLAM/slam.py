...
class wikipedia_preprocessor:

    def __init__(self, verbose) -> None:
        self.verbose = verbose

    def preprocess_wikipedia(self, wiki_data, tokenizer=None, max_length=1024):
        """
        Complete pipeline to prepare Wikipedia data for next-token prediction.

        Args:
            wiki_data: Raw Wikipedia text or list of articles
            tokenizer: Optional tokenizer to use for length calculations
            max_length: Maximum sequence length (in tokens if tokenizer provided, characters otherwise)
        ...
        if isinstance(wiki_data, list):
            cleaned_articles = [
                self._preprocess_wiki_article(article) for article in wiki_data
            ]
            cleaned_articles = [
                self._clean_wiki_references(article)
                for article in cleaned_articles
            ]
        else:
            cleaned_articles = self._preprocess_wiki_article(wiki_data)
            cleaned_articles = self._clean_wiki_references(cleaned_articles)

        # 2. Split into chunks
        if tokenizer:
            # If we have a tokenizer, calculate chunk size in tokens
            chunk_size = max_length
            stride = max_length // 4  # 25% overlap
        else:
            # Otherwise use character count as approximation
            # Average English word is ~5 chars, and tokens can be smaller
            chunk_size = max_length * 4  # Rough approximation
            stride = chunk_size // 4

        chunks = self.process_wikipedia_data(
            cleaned_articles, chunk_size, stride
        )

        # 3. Add structure markers if desired
        formatted_chunks = self._format_wiki_for_training(chunks)

        return formatted_chunks

    def _clean_wiki_references(self, text):
        """Remove or simplify Wikipedia references"""
    ...
    def _format_wiki_for_training(self, chunks):
        """Add special tokens to help the model understand document structure"""
        formatted_chunks = []

        for chunk in chunks:
            # Detect if this chunk has a title/heading at the start
            if chunk.startswith("= ") or chunk.startswith("== "):
                # It starts with a heading
                formatted_chunk = "<|section|> " + chunk
            else:
                # Regular paragraph
                formatted_chunk = "<|paragraph|> " + chunk

            formatted_chunks.append(formatted_chunk)

        return formatted_chunks

    def _preprocess_wiki_article(self, article_text):
        """Clean and structure a Wikipedia article"""
        # Remove edit links [edit] that appear in Wikipedia content
        cleaned = re.sub(r"\[\s*edit\s*\]", "", article_text)

        # Handle section headings properly
        cleaned = re.sub(r"(=+)\s*(.*?)\s*\1", r"\n\1 \2 \1\n", cleaned)
    ...
    def process_wikipedia_data(self, wiki_text, chunk_size=1024, stride=256):
        """
        Process Wikipedia text into suitable chunks for next-token prediction.

        Args:
            wiki_text: Raw Wikipedia text or list of Wikipedia articles
        ...
        # If input is a list of articles
        if isinstance(wiki_text, list):
            # Process each article
            for article in wiki_text:
                article_chunks = self._chunk_wiki_article(
                    article, chunk_size, stride
                )
                chunks.extend(article_chunks)
        else:
            # Process single text
            chunks = self._chunk_wiki_article(wiki_text, chunk_size, stride)

        return chunks

    def _chunk_wiki_article(self, article_text, chunk_size=1024, stride=256):
        """
        Split a Wikipedia article into appropriate chunks while respecting natural boundaries.

        Args:
            article_text: Text of a single Wikipedia article
        ...
        # 1. Try to respect paragraph boundaries first
        paragraphs = [
        ...
        for para in paragraphs:
            ...
            if len(current_chunk) + len(para) > chunk_size and current_chunk:
                ...
                # Keep last few sentences for context (if they exist)
                overlap_text = (
                    ". ".join(sentences[-3:]) if len(sentences) > 3 else ""
                )
                current_chunk = (
                    overlap_text + " " + para if overlap_text else para
                )
            else:
        ...
        # 2. If chunks are still too large, use sliding window approach
        final_chunks = []
        for chunk in chunks:
            if len(chunk) <= chunk_size:
                final_chunks.append(chunk)
            else:
                # Use sliding window for very large chunks
                for i in range(0, len(chunk) - stride, stride):
                    # Make sure we don't create tiny chunks at the end
                    end_pos = min(i + chunk_size, len(chunk))
                    window = chunk[i:end_pos]
